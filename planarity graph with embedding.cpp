...
public:
    ...
    static void layChain(vector<vector<bool>>* result, vector <int> chain, bool cyclic) {
            for (int i = 0; i < chain.size() - 1; i++) {
                (*result)[chain[i]][chain[i + 1]] = true; //var-create: unable to create variable object
    ...
    vector <int> calcNumOfFacesContainedSegments (vector<vector<int>> *intFaces, const vector<int>& extFace,
                                                 vector <Graph> segments, vector<bool>* laidVertexes,vector<vector<int>>* destFaces) const {
    ...
    Faces* getPlanarLaying() {
        ...
        while (true) {
            ...
            auto destFaces = new vector<vector<int>>(segments->size(), vector<int> (segments->size()));
            ...
            if (count[mi] == 0) {
                ...
                vector<int> face = destFaces[mi][mi];
                ...
                if (face != *extFace) {
                    ...
                    for (int i = 0; i < face1->size(); i++) {
                        intFaces->push_back(face1[i]);
                    }
                    for (int i = 0; i < face2->size(); i++) {
                        intFaces->push_back(face2[i]);
                    }
